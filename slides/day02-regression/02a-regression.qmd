...
## Overview (To be removed)
- Regression Activity based on Penguin data
- Regression Lecture based on Babies data

<!--
## Linear Regression Activity: Penguin Data (see Markdown file)
https://mdsr-book.github.io/mdsr2e/ch-foundations.html#sec:gooddesign

- We have used the penguins data to model the length of penguin flippers in mm (Y) by the length of their bills in mm (X). Fit this model again and call it model_bill_length.
- Fit another model with and call it model_bill_length_depth. The explanatory variables in this model should be bill_length_mm and bill_depth_mm.
- Which model has a better fit model_bill_length or model_bill_length_depth? Compare relevant model evaluation criteria.
